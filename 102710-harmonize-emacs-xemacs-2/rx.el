...
(defmacro rx-char-p (x)
  "Test if x is a character"
  (list (if (featurep 'xemacs) 'characterp 'numberp) x))
...
(defun rx-check-any (arg)
   ...
   (cond
    ...
    ((and (rx-char-p (car-safe arg)) (rx-char-p (cdr-safe arg)))
     (list arg))
     ...
     ((or (eq (car args) ?^) (eq (car-safe (car args)) ?^))
      (setq args (if (cdr args)
		   ...
		   (nconc (rx-any-delete-from-range ?^ args)
			  (list ?^))))))
    ;; some 1-char?
    (if (and (null (cdr args)) (rx-char-p (car args))
	     (or (= 1 (length
...
(defun rx-category (form)
  ...
  (rx-check form)
  (let ((char (if (rx-char-p (cadr form))
		  (cadr form)
